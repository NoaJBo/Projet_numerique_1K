...
def f_derivee_seconde_carre(x):
    """
    Retourne la dérivée seconde de la fonction x^2 => 2 (constante)
    """
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------
    # 3.1.2 : Reprise des questions précédentes pour la dérivée seconde
    # ---------------------------------------------------------
    ...
    ax2[0].set_title("Comparaison des dérivées secondes")
    ax2[0].legend()

    ax2[1].plot(x, erreur_seconde, color="black")
    ax2[1].set_title("Erreur absolue de la dérivation seconde")

    plt.tight_layout()
    plt.savefig("Dérivée_seconde.png")
