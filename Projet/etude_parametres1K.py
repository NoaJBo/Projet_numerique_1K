...
# Configuration pour ton système Arch Linux
...
def simuler_silencieux(Re: np.ndarray, Im: np.ndarray, V: np.ndarray, dx: float, dt: float, nt: int) -> np.ndarray:
    """
    Moteur physique pur. Fait exactement les mêmes calculs que la fonction
    animée, mais sans Matplotlib pour calculer des milliers de fois plus vite.
    """
    for _ in range(nt):
...
if __name__ == "__main__":
    ...
    k0_fixe = 2.8
    largeur_paquet = 2.0  # Nommé largeur_paquet pour ne pas confondre avec 'a' (largeur du puits)
    xc = -20.0
    debut_barriere = 5.0
    
    # ---------------------------------------------------------
    # ÉTUDE 1 : INFLUENCE DE 'a' (Largeur du puits)
    # ---------------------------------------------------------
    print("Calcul de l'influence de la largeur 'a' en cours...")
    V0_fixe = 10.0
    # On va tester 30 largeurs de puits différentes entre 1 et 10
    ...
    # ---------------------------------------------------------
    # ÉTUDE 2 : INFLUENCE DE 'V0' (Profondeur du puits)
    # ---------------------------------------------------------
    print(f"\nCalcul de l'influence de V0 en cours...")
    a_fixe = 5.0
    # On va tester 30 profondeurs différentes entre -1 et -20
    ...
    # Graphique 2 : Influence de 'V0'
    # On inverse l'axe X pour que la profondeur augmente vers la droite
