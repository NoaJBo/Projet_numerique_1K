...
def simuler_et_animer(x: np.ndarray, dx: float, dt: float, nt: int, V: np.ndarray, psi_initial: np.ndarray) -> np.ndarray:
    ...
    Stocke l'historique complet dans des matrices 2D (nx, nt) et gère l'affichage.
    """
    nx = len(x)

    # 1. Allocation des matrices 2D pour stocker tout l'historique (remplies de zéros)
    Re_2D = np.zeros((nx, nt))
    Im_2D = np.zeros((nx, nt))

    # 2. Initialisation de la première colonne (instant t = 0, indice n = 0)
    Re_2D[:, 0] = np.real(psi_initial)
    Im_2D[:, 0] = np.imag(psi_initial)

    # Configuration de la fenêtre Matplotlib pour l'animation
    plt.ion()
    fig, ax = plt.subplots(figsize=(10, 6))

    densite_initiale = Re_2D[:, 0]**2 + Im_2D[:, 0]**2
    ligne_densite, = ax.plot(x, densite_initiale, label="Densité de probabilité |Psi|²", color="blue", linewidth=2)
    ligne_potentiel, = ax.plot(x, V / 5.0, label="Potentiel V(x) (échelle 1:5)", color="orange", linestyle="--")

    ax.set_xlim(x[0], x[-1])
    ax.set_ylim(-1.2, 1.5)
    ax.set_title("Effet Tunnel : Propagation avec Stockage Historique 2D")
    ax.set_xlabel("Position x")
    ax.set_ylabel("Amplitude")
    ax.legend(loc="upper right")
    ax.grid(True)

    print("Début de la simulation avec stockage 2D...")

    # 3. Boucle temporelle : on s'arrête à nt - 1 pour ne pas dépasser la taille de la matrice
    for n in range(nt - 1):
        # On calcule la dérivée seconde spatiale en lisant la colonne actuelle (n)
        d2_Re = derivee_seconde_num(Re_2D[:, n], dx)
        # On remplit la colonne suivante (n+1) pour la partie imaginaire
        Im_2D[:, n+1] = Im_2D[:, n] + (dt / 2.0) * d2_Re - dt * V * Re_2D[:, n]

        # On utilise la nouvelle colonne calculée (n+1) pour la partie imaginaire
        d2_Im = derivee_seconde_num(Im_2D[:, n+1], dx)
        # On remplit la colonne suivante (n+1) pour la partie réelle
        Re_2D[:, n+1] = Re_2D[:, n] - (dt / 2.0) * d2_Im + dt * V * Im_2D[:, n+1]

        # Rafraîchissement visuel toutes les 200 itérations
        if n % 200 == 0:
            densite_actuelle = Re_2D[:, n+1]**2 + Im_2D[:, n+1]**2
            ligne_densite.set_ydata(densite_actuelle)
            plt.pause(0.001)

    print("Simulation terminée ! Historique complet sauvegardé.")
    plt.ioff()

    # La densité finale correspond à la toute dernière colonne (-1) de nos matrices
...
if __name__ == "__main__":
    ...
    # Tableau 1d pour l'intervalle de temps (3.2.3 du sujet, demandé au même
    # titre que x). On le construit avec linspace puis on vérifie qu'il est
    # bien cohérent avec le pas dt utilisé dans la boucle de simulation.
